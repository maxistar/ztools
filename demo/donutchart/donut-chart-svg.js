import { h, signal, computed, tags as t } from "../../dist/ztools.client.full.js";
...
export function DonutChart({ data, width = 760, height = 360, outerR = 110, innerR = 62 }) {
  ...
  return t.div(
    { style: { display: "grid", gridTemplateColumns: "auto 1fr", gap: "28px", alignItems: "start", fontFamily: "system-ui, sans-serif" } },
    s.svg(
      {
        attrs: { width, height, viewBox: `0 0 ${width} ${height}` },
        style: { overflow: "visible", display: "block" },
      },
      s.text({ attrs: { x: 20, y: 28, fill: "#111" }, style: { fontSize: "18px", fontWeight: "600" } }, "Traffic sources"),
      () => slices().map((slice) => {
        const isActive = () => hovered() === slice.index;
        return s.g(
          s.path({
            attrs: { d: slice.path, fill: slice.color, stroke: "#fff", "stroke-width": () => (isActive() ? 4 : 2) },
            style: { cursor: "pointer", transition: "transform 120ms ease, opacity 120ms ease" },
            opacity: () => {
              const h = hovered();
      ...
      s.text({ attrs: { x: cx, y: cy - 8, "text-anchor": "middle", fill: "#666" }, style: { fontSize: "12px", fontWeight: "500" } }, "Total"),
      s.text({ attrs: { x: cx, y: cy + 18, "text-anchor": "middle", fill: "#111" }, style: { fontSize: "28px", fontWeight: "700" } }, () => total()),
      () => {
        ...
        return s.g(
          ...
          s.rect({ attrs: { x, y, rx: 10, ry: 10, width: boxW, height: boxH, fill: "white", stroke: "#ddd" } }),
          s.text({ attrs: { x: x + 12, y: y + 20, fill: "#111" }, style: { fontSize: "12px", fontWeight: "700" } }, active.label),
          s.text({ attrs: { x: x + 12, y: y + 38, fill: "#555" }, style: { fontSize: "12px" } }, `${active.value} (${active.percent}%)`),
        );
      },
    ),
    t.div(
      ...
      () =>
        slices().map((slice) =>
          t.div(
            ...
            t.div({ style: { width: "14px", height: "14px", borderRadius: "4px", background: slice.color } }),
            t.div(slice.label),
            t.div({ style: { opacity: 0.78, fontVariantNumeric: "tabular-nums" } }, `${slice.value} (${slice.percent}%)`),
          ),
        ),
      t.div({ style: { marginTop: "18px", paddingTop: "12px", borderTop: "1px solid #eee", fontWeight: "600" } }, "Total: ", () => total()),
    ),
  );
}
